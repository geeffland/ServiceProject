...
//		ServiceProjectHelper::addSubmenu(JRequest::getWord('view', $this->default_view));
	 ...
	 * @since	1.6
	 */
	public function getRegistrationStatusModel($name = 'RegStatus', $prefix = 'ServiceProjectModel', $config = array('ignore_request' => true))
	{
		return parent::getModel($name, $prefix, $config);
	}

	public function toggleRegistrationIsCounted()
	{
		// Check for request forgeries. // Disable for now because I don't know how to pass a token thru an Ajax call.
		JRequest::checkToken() or jexit(JText::_('JINVALID_TOKEN'));
		$messages = array();
		...
		require_once JPATH_COMPONENT.'/helpers/serviceproject.php';
		require_once JPATH_COMPONENT.'/models/regstatus.php';
		// Initialise variables.
		$ids	= JRequest::getVar('cid', array(), '', 'array');
		$prefix	= JRequest::getVar('prefix', '');
		$jsTask	= JRequest::getVar('jstask', '');
		...
		if (empty($ids)) {
			$messages[] = JText::_('COM_SERVICEPROJECT_REGISTRATION_STATUS_NO_ITEM_SELECTED');
		} else {
			// Get the model.
			$model = $this->getRegistrationStatusModel();

			//cycle thru ids one at a time so html and messages can be collected
			foreach ($ids as $id) {
				// Change the state of the records.
				if (!$model->toggleRegistrationIsCounted($ids, $prefix, $retValue)) {
					$messages[] = $model->getError();
				} else {
					$htmlcode = ServiceProjectHelper::showTrueFalseLink($retValue, $jsTask, $prefix, $id, 'COM_SERVICEPROJECT_ISCOUNTED', 'COM_SERVICEPROJECT_ISNOTCOUNTED');
					$html[] = array('id'=>$prefix.$id, 'html'=>$htmlcode);
				}
			}
		}
