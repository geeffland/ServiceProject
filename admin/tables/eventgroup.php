...
require_once JPATH_COMPONENT.DS.'tables'.DS.'archiveeventgroup.php';
 ...
 * @subpackage	admin.tables
 */
class ServiceProjectTableEventGroup extends JTable
{
	protected $tabletype 	= 'EventGroup';
	...
	{
		parent::__construct('#__serviceproject_eventgroups', 'id', $_db);
	...
	function check()
	{
		...
		// Check Registration date is before start
//		if (intval($this->registration_startdate) > 0 && $this->registration_startdate >= $this->startdate) {
//			// Warn that registraion is after the start date
//				$this->setError(JText::_('COM_SERVICEPROJECT_ERROR_REGISTRATIONAFTERSTART'));
//				return false;
//		}
	...
	function store($updateNulls = false)
	{
		...
		if (empty($this->id))
		{
			...
			$this->eventgroup_id 		= ServiceProjectHelper::getNewEventGroupID();
			// Store the row
			parent::store($updateNulls);
			ServiceProjectHelper::setNewEventGroupID($this->eventgroup_id + 1);
		}
		else
		{
			// Get the old row
			...
			$ArchiveTable = new ServiceProjectTableArchiveEventGroup($this->_db);
